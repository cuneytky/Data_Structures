...
public class MyTree { // 0- verilerimi tutacak bir Node kalsı belirledim sonra buraya geldim:
    TNode root; // 1- öncelikle bir root ihtiyacım var

    public MyTree() { // 2- sonrasında bir cons. ihtiyacım var
        this.root = root;
    }
    void insert(int value){
 // yeni node oluşturduk
        TNode newNode=new TNode(value);
 // ilk durum => if tree is empty ise
        if (root==null) {
            root=newNode;
            return;
        }
// 2. durum => if tree is not empty ise
        TNode current =root;
        while(true){
    ...
    void preOrderTraversal(TNode root){
      ...
      // interview question ==> inOrder ve deletion part
     // exlplain balance =>  if right and left balance difference is greater than 1 we can say there is an imbalance in the tree. That cause more time complexity compare to balance tree
    void inOrderTraversal(TNode root){  // ascending => her zaman mı kontrol et
        if (root==null) return; // termination
